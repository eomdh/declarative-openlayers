...
전북 김제 농경지 필지를 세 구획으로 나눠 벡터 레이어 셋으로 올리고, 배경은 OSM 타일이다. 오른쪽 목록이 곧 렌더 트리다.

- 레이어를 토글하면 그것만 사라진다.
- 위아래로 옮기면 zIndex 가 바뀌어 겹침 순서가 실시간으로 바뀐다.
- 배경 지도는 항상 맨 아래에 고정된다.

필지 데이터(`public/parcels.geojson`)는 GIS 짝 repo [browser-geotiff-epsg5179](https://github.com/eomdh/browser-geotiff-epsg5179) 의 것을 그대로 쓴다. 그쪽은 GeoTIFF 디코딩을, 이쪽은 레이어 추상화를 맡는다. `custom` 종류로 그 repo 가 만든 래스터 레이어를 그대로 얹을 수도 있다.
